...
namespace simtools
{
	...
	// growing_binary_search is a binary search tree that allows the addition and removal of events. In the case of addition, 
	// the new event is appended to the list of already existing possible events. If this list is at its size limit, the tree
	// is checked for already removed items at the beginning. If there are any, all entries are renumbered so that what before
	// was the first nonempty entry of the list in now the first entry. 
	// In case there are no empty (i.e. previously destroyed) elements at the beginning, the size of the data field is in-
	// creased to the next power of two.
