...
//	Daniel Löb, 2011
//	
//	Heavyly inspired by the english wikipedia page on red-black trees.
//
///////////////////////////////////////////////////////////////////////////////
...
template <typename d_t> ostream& operator<<(ostream& the_stream, const red_black_tree<d_t> &rbt)
{
	...
	else
	{
		...
		while(true)
		{
			...
			the_stream //<<(*tmpit)->depth << "\t" <<  bitset<sizeof(unsigned long)*8>((*tmpit)->position) << "\t" 
			<< (*tmpit)->family_members << "\t" << (*tmpit)->red << "\t" << (*tmpit)->value << endl;
...
// 	the_stream << recursive_output(rbt.tree);
